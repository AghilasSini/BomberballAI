...
/* Implémentation de l'algorithme alpha-béta en version récursive avec limitation de profondeur et heuristique*/

public class IAImplementation2 extends AbstractAI {
    ...
    private int profondeur = 12;


    public IAImplementation2(GameConfig config, String player_skin, int playerId) {
        super(config, player_skin, "CustomAI2", playerId);
    }

    @Override
    public Action choosedAction(GameState gameState) {
        alphabeta(gameState, this.alpha, this.beta, 0);
    ...
    private int calcHeuristic(GameState etat) {
        ...
        int distance = (int) (player0.getCell().distanceTo(player1.getCell()) * 10);
        return 500-distance;
    }
